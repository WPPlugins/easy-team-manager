    ...
    <section class="the_team" style=" <?php if($setting_list['img_set']!="unset"){ $upload_dir=wp_upload_dir();?> background:url(<?php echo $upload_dir["baseurl"]."/"."easy_team_manager/".$get_result->background_image;?>);<?php } else {?> background:<?php echo $setting_list['background_color'];?>;<?php }?>">
        <div>
            ...
            <div class="clearfix"></div>            
                <?php global $wpdb;
            $rw = $wpdb->get_results("SELECT * from ".$wpdb->prefix."easy_team_manager_description INNER JOIN ".$wpdb->prefix."easy_team_manager where ".$wpdb->prefix."easy_team_manager.tid = "."".$wpdb->prefix."easy_team_manager_description.team_id"." AND ".$wpdb->prefix."easy_team_manager.team_name='".$atts['team_name']."'");
